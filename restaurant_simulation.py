...
class Group:
    #Store a customer group's arrival time, size and dining duration.
...
class TableUnit:
    #The class of a table in the restaurant.
...
class CombinedTable:
    #A class of combination of 2 tables.
...
class SimulationResult:
    #Store output metrics and group records of one specific simulation mode.
...
class RestaurantSimulator:
    #Run Mode 1&Mode 2 restaurant simulations.
    ...
    def _build_tables(self):
        #Create tables according to the table arrangement.
        tables = []
        table_id = 1
    ...
    def _generate_groups(
        ...
        raise ValueError(
            "This version requires CSV input. Please select a CSV file with columns: arrival_time, group_size, dining_time")

    def _load_groups_from_csv(self, path):
        #Load customer groups from a CSV file.
        groups = []
        with open(path, "r", encoding="utf-8", newline="") as f:
            reader = csv.DictReader(f)
            required = {"arrival_time", "group_size", "dining_time"}
            if not required.issubset(reader.fieldnames or []):
                raise ValueError("CSV must contain columns: arrival_time, group_size, dining_time")
    ...
    def get_groups(
        #get customer groups and return data
            self,
            use_csv,
            csv_path,
            total_groups,
    ...
    def _finalize_remaining_waiting(self, waiting, result, current_time):
        #Mark all the remaining waiting customer groups as "unserved" under particular circumstances.
        while waiting:
    ...
    def run_mode_2(self, groups):
        #Run Mode 2
        result = SimulationResult("Mode 2: Dedicated Size-Based Queues")
        result.total_groups = len(groups)
        result.total_customers = sum(g.group_size for g in groups)
        result.size_distribution = Counter(g.group_size for g in groups)
        ...
        queue_small = deque()  # groups of 1-2 people
        queue_medium = deque()  # groups of 3-4 people
        queue_large = deque()  # groups of 5-6 people

        small_tables = [t for t in tables if t.capacity == 2]
        medium_tables = [t for t in tables if t.capacity == 4]
        large_tables = [t for t in tables if t.capacity == 6]
        ...
        # Get a queue for a group
        def get_queue_for_group(group):
            if group.group_size <= 2:
                return queue_small
            elif group.group_size <= 4:
                return queue_medium
            else:
                return queue_large

        #Get tables for a queue type
        def get_tables_for_queue(queue_type):
            if queue_type is queue_small:
                return small_tables
            elif queue_type is queue_medium:
        ...
        while current_time <= self.simulation_minutes or idx < len(groups):
            while idx < len(groups) and groups[idx].arrival_time <= current_time:
                group = groups[idx]
                target_queue = get_queue_for_group(group)
                target_queue.append(group)
                idx += 1

            all_queues = [queue_small, queue_medium, queue_large]
            all_tables = [small_tables, medium_tables, large_tables]

            total_served = 0
            served_waits = []

            for queue, table_list in zip(all_queues, all_tables):
                new_queue = deque()
                ...
                while queue:
                    ...
                    #Find free tables that can hold this group
                    free_tables = [t for t in table_list if t.is_free(current_time) and t.capacity >= group.group_size]

                    if free_tables:
                        #Choose the best-fit table
                        best_table = min(
                            free_tables,
                            key=lambda t: (t.capacity - group.group_size, t.capacity, t.table_id)
                        )
                        ...
                        result.total_revenue += group.group_size * self.revenue_per_customer
                        total_served += 1
                        served_waits.append(waited)

                        queue_name = "Small(1-2)" if queue is queue_small else (
                            "Medium(3-4)" if queue is queue_medium else "Large(5-6)")
                        ...
                        result.group_records.append({
                            ...
                            "wait_time": round(waited, 2),
                            "served": 1,
                            "assigned_table": f"{queue_name}:T{best_table.table_id}(cap={best_table.capacity})",
                        })
                    else:
                        new_queue.append(group)
            ...
            avg_wait_now = sum(served_waits) / len(served_waits) if served_waits else 0
            result.timeline_wait.append((current_time, avg_wait_now))

            total_waiting = len(queue_small) + len(queue_medium) + len(queue_large)
            result.timeline_queue.append((current_time, total_waiting))

            current_time += 1

            # Check whether simulation should come to an end
            if current_time > self.simulation_minutes and idx >= len(groups):
                all_queues_empty = all(len(q) == 0 for q in all_queues)
                if all_queues_empty:
                    break

        # Handle remaining groups at the end
        all_queues = [queue_small, queue_medium, queue_large]
...
class RestaurantApp:
    #(Tkinter GUI)App that can set inputs, run simulations, and show outputs.
    ...
    def _init_zoom_fonts(self):
        #Store base font sizes so that the interface can be zoomed safely.
    ...
    def _build_ui(self):
        ...
        ttk.Button(button_frame, text="Export Mode 1 CSV", command=lambda: self.export_result(1)).pack(side="left",
                                                                                                       padx=6)
        ttk.Button(button_frame, text="Export Mode 2 CSV", command=lambda: self.export_result(2)).pack(side="left",
                                                                                                       padx=6)
    ...
    def run_simulation(self):
        #Read inputs, run modes and update outputs(charts).
        try:
    ...
    def show_result(self, widget, result):
        #Show summary metrics for a simulation mode.
    ...
    def _avg_wait_by_group_size(self, result):
        #Return average waiting time for served groups.
        grouped_waits = {}
    ...
    def _draw_text_table(self, x, y, title, headers, rows, col_widths, row_height=24):
        ...
        for header, width in zip(headers, col_widths):
            ...
            self.canvas.create_text(current_x + 6, y + row_height / 2, text=header, anchor="w",
                                    font=self._canvas_font(9, "bold"))
            current_x += width
        y += row_height
        for row in rows:
            current_x = x
            for value, width in zip(row, col_widths):
                self.canvas.create_rectangle(current_x, y, current_x + width, y + row_height, outline="black")
                self.canvas.create_text(current_x + 6, y + row_height / 2, text=str(value), anchor="w",
    ...
    def draw_visualization(self, r1, r2):
        #Draw bar charts,queue-length curves and summary tables.
        ...
        self.canvas.create_rectangle(left_x + 170, legend_y, left_x + 190, legend_y + 15, fill="#C0504D",
                                     outline="black")
        ...
        all_sizes = sorted(
            set(avg_by_size_1) | set(avg_by_size_2) | set(r1.size_distribution) | set(r2.size_distribution))
        group_size_rows = []
