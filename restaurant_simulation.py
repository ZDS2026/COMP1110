...
class Group:
    """Store one customer group's arrival time, size, and dining duration."""
...
class TableUnit:
    """Represent a single physical table in the restaurant."""
...
class CombinedTable:
    """Represent a temporary combination of two tables. Kept for compatibility."""
...
class SimulationResult:
    """Store all output metrics and detailed group records for one simulation mode."""
...
class RestaurantSimulator:
    """Run Mode 1 and Mode 2 restaurant seating simulations."""
    ...
    def _build_tables(self):
        """Create table objects according to the user-defined table configuration."""
        tables = []
        table_id = 1
    ...
    def run_mode_1(self, groups):
        """Mode 1: all groups use one common queue and any suitable table can be used."""
    ...
    def run_mode_2(self, groups):
        """Mode 2: groups are split into dedicated size-based queues and table types."""
        ...
        # Create 3 dedicated queues based on group size
        queue_small = deque()    # groups of 1-2 people
        queue_medium = deque()   # groups of 3-4 people  
        queue_large = deque()    # groups of 5-6 people
        
        # Assign tables to specific queue types
        # Small tables (capacity 2) go to small queue
        # Medium tables (capacity 4) go to medium queue
        # Large tables (capacity 6) go to large queue
        ...
        # Helper function to get the appropriate queue for a group
        ...
        # Helper function to get tables for a queue type
        ...
        while current_time <= self.simulation_minutes or idx < len(groups):
            # Add newly arrived groups to their dedicated queues
            while idx < len(groups) and groups[idx].arrival_time <= current_time:
                group = groups[idx]
            ...
            # Process each queue with its dedicated tables
            for queue, table_list in zip(all_queues, all_tables):
                ...
                while queue:
                    ...
                    # Find free tables that can accommodate this group
                    free_tables = [t for t in table_list if t.is_free(current_time) and t.capacity >= group.group_size]
                    
                    if free_tables:
                        # Choose the table that fits best (minimize waste)
                        best_table = min(
                            free_tables,
                            key=lambda t: (t.capacity - group.group_size, t.capacity, t.table_id)
                        )
                        ...
                        # Determine queue name for display
                        queue_name = "Small(1-2)" if queue is queue_small else ("Medium(3-4)" if queue is queue_medium else "Large(5-6)")
                        ...
                        # No free table available, keep in queue
                        new_queue.append(group)
            ...
            # Calculate average wait time for this time step
        ...
        # Handle any remaining groups at the end of simulation
        all_queues = [queue_small, queue_medium, queue_large]
...
class RestaurantApp:
    """Tkinter GUI for setting inputs, running simulations, and showing outputs."""
    ...
    def run_simulation(self):
        """Read inputs, run both modes, and update the text outputs and charts."""
        try:
    ...
    def show_result(self, widget, result):
        """Display summary metrics for one simulation mode."""
    ...
    def draw_visualization(self, r1, r2):
        """Draw bar charts, queue-length curves, and summary tables on the canvas."""
